...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  /* 
  * If the sql string contains "' or " it is an indication of SQL injection
  * We can remove the single quote ' to avoid it
  */
  const char posOr = sanitizedSQL.find("' or ");

  if (posOr < sanitizedSQL.length()) {
      isSQLinjection = true;

      sanitizedSQL.erase(posOr, 1);

      /*
      * As an additional step, to make sure that the string is safe,
      * we can check if it also contains '=' which is also sign of SQL injection
      * We can remove the equal sign '=' as an extra precaution
      */
      const char posEqual = sanitizedSQL.substr(posOr).find('=') + posOr;

      if (posEqual < sanitizedSQL.length()) {
          sanitizedSQL.erase(posEqual, 1);
      }
  }

  char* error_message;
  if(sqlite3_exec(db, sanitizedSQL.c_str(), callback, &records, &error_message) != SQLITE_OK)
  {
    std::cout << "\nData failed to be queried from USERS table. ERROR = " << error_message << std::endl;
    if (isSQLinjection) {
        std::cout << "Indication of SQL Injection: " << sql.substr(sql.find("'")) << std::endl;
    }
    sqlite3_free(error_message);
    return false;
  }
