...
{
  std::string file_text = "Diego Bez Zambiazzi\n";
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //  Line 4+: data
    ...
    // Timestamp: Use localtime_s to convert from time in seconds into tm struct
    ...
    if (fileStream.is_open()) {
        fileStream << student_name << "\n";

        // YYYY = tm_year + 1900
        //  MM  = tm_mon + 1
        //  DD  = tm_mday
        fileStream << now_time.tm_year + 1900 << "-" << now_time.tm_mon + 1 << "-" << now_time.tm_mday << "\n";
