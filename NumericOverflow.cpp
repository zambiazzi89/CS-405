...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        /*
         * result + increment <= max
         * If (result > max - increment) Overflow
         */

        // If overflow, throws exception with wrong result
...
{
    /*
     * Equation result must be less-than or equal-to type's min:
     * start - decrement * steps <= min
     * Underflow if: start/steps - decrement > min/steps
     */
    ...
    {
        /*
         * result - decrement >= min
         * If (result < min + decrement) Underflow
         */

        // If underflow, throws exception with wrong result
...
void test_overflow()
{
    ...
    std::cout << "Overflow Test of Type = " << typeid(T).name() << ", Max range: " << +std::numeric_limits<T>::max() << std::endl;
    // END DO NOT CHANGE

    try {
        // This test should run without issues
        std::cout << "\tAdding Numbers Without Overflow (" << +start << ", " << +increment << ", " << steps << ") = ";
        T result = add_numbers<T>(start, increment, steps);
        std::cout << +result << std::endl;

        // This test should throw an exception that will be catched below
        std::cout << "\tAdding Numbers With Overflow (" << +start << ", " << +increment << ", " << (steps + 1) << ") = ";
        result = add_numbers<T>(start, increment, steps + 1);
        std::cout << +result << std::endl;
    }
    // Catch exceptions and inform the user
    catch (T res) {
        std::cerr << "\n\t\tOverflow occurred!\n\t\t" <<
            +start << " + " << +increment << " * " << (steps + 1) << " > " << +std::numeric_limits<T>::max() <<
            "\n\t\tOverflow Result: " << +res << std::endl;
    }
}
...
void test_underflow()
{
    ...
    std::cout << "Underflow Test of Type = " << typeid(T).name() << ", Min range: " << +std::numeric_limits<T>::min() << std::endl;
    // END DO NOT CHANGE
    ...
    try {
        ...
        // This test should (but sometimes doesn't) throw an exception that will be catched below
        std::cout << "\tSubtracting Numbers With Underflow (" << +start << ", " << +decrement << ", " << (steps + 1) << ") = ";
        result = subtract_numbers<T>(start, decrement, steps + 1);
        std::cout << +result << std::endl;

        // New test to throw an exception that will be catched below
    ...
    // Catch exceptions and inform the user
        ...
        std::cerr << "\n\t\tUnderflow occurred!\n\t\t" <<
            +start << " - " << +decrement << " * " << (steps + 1) << " < " << +std::numeric_limits<T>::min() <<
            "\n\t\tUnderflow Result: " << +res << std::endl;
    }
}
